...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $stmt = $connection->prepare("SELECT * FROM player_mail WHERE name LIKE :search");
    $stmt->bindValue(':search', '%' . $search . '%');
} else {
    $stmt = $connection->prepare("SELECT * FROM player_mail");
}
$stmt->execute();
...
<body>
    <div>
        <h1 class="text-center bg-success text-white p-2">Player Email List</h1>
    </div>
